...
ax = axes[0, 0]
methods_names = list(all_metrics.keys())
elim_acc = [all_metrics[m]['淘汰准确率'] for m in methods_names]
colors = ['#3498db', '#e74c3c', '#9b59b6', '#2ecc71']
bars = ax.bar(methods_names, elim_acc, color=colors, alpha=0.7, edgecolor='black')
...
# Champion Accuracy
ax = axes[0, 1]
champ_acc = [all_metrics[m]['冠军准确率'] for m in methods_names]
bars = ax.bar(methods_names, champ_acc, color=colors, alpha=0.7, edgecolor='black')
ax.set_ylabel('Accuracy', fontsize=12)
...
# Top3 Accuracy
ax = axes[1, 0]
top3_acc = [all_metrics[m]['Top3准确率'] for m in methods_names]
bars = ax.bar(methods_names, top3_acc, color=colors, alpha=0.7, edgecolor='black')
ax.set_ylabel('Accuracy', fontsize=12)
...
ax = axes[1, 1]
spearman = [all_metrics[m]['Spearman相关'] for m in methods_names]
bars = ax.bar(methods_names, spearman, color=colors, alpha=0.7, edgecolor='black')
...
# 5.2 Controversial Player Comparison
...
for i, col in enumerate(pivot_data.columns):
    ax.bar(x + i*width, pivot_data[col], width, label=col, color=colors[i], alpha=0.7, edgecolor='black')
...
# 5.3 KMDR Component Distribution
fig, axes = plt.subplots(2, 2, figsize=(14, 10))
...
# Right: Controversial players distortion boxplot
ax = axes[1]
controversy_data = []
controversy_labels = []
for name in controversy_names:
    ...
    if len(data) > 0:
        ...
        controversy_labels.append(name.split()[0])  # Only take first name
