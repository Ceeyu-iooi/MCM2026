...
class ArrowTheoremVerifier:
    ...
    阿罗不可能定理指出，不存在完美的投票系统能同时满足：
    1. 帕累托效率 (Pareto Efficiency): 如果所有人偏好A>B，结果应为A>B
    2. 非独裁性 (Non-Dictatorship): 没有单一投票者决定结果
    3. 独立性 (IIA): 第三方选手不应影响A与B的相对排名
    
    我们检验DWTS的两种方法对这些条件的满足程度
    """
    ...
    def check_iia(self):
        """
        检验独立性 (IIA - Independence of Irrelevant Alternatives)
        
        第三方选手的加入/退出不应改变其他两人的相对排名
        这在排名法中容易被违反
        """
        # 排名法天然违反IIA（排名取决于其他选手）
        # 百分比法部分违反（总分影响百分比）
...
width = 0.35
ax2.bar(x - width/2, rank_scores, width, label='Rank Method', color='#1976D2', alpha=0.8)
ax2.bar(x + width/2, pct_scores, width, label='Percentage Method', color='#F57C00', alpha=0.8)
ax2.set_xticks(x)
...
width = 0.35
ax4.bar(x - width/2, rank_vals, width, label='Rank Method', color='#1976D2', alpha=0.8)
ax4.bar(x + width/2, pct_vals, width, label='Percentage Method', color='#F57C00', alpha=0.8)
ax4.set_xticks(x)
ax4.set_xticklabels(dimensions)
ax4.set_ylabel('Score (max 25)', fontsize=11)
