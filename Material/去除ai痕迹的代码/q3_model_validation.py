...
if df['week'].dtype == 'object':
    df['week_num'] = pd.to_numeric(df['week'].str.extract(r'(\d+)')[0], errors='coerce')
else:
...
if X.shape[1] != len(feature_names):
    print(f"[警告] SimpleImputer 删除了全为空的列. 原始: {len(feature_names)}, 现在: {X.shape[1]}")
    # 找出非全空的列
    valid_mask = ~df[feature_names].isna().all()
    feature_names = [name for name, valid in zip(feature_names, valid_mask) if valid]
    print(f"[修正] 更新后的特征列表: {feature_names}")
...
# 使用名称映射而非硬编码索引，以应对特征被移除的情况
metrics_groups = {
    '舞者特征': ['wins_std', 'seasons_std', 'avg_place_std', 'partner_is_champion'],
    '明星特征': ['age_std', 'followers_std', 'celebrity_has_follower_data', 'age_sq_std'],
    '时间特征': ['season_std', 'week_std'],
    '交互特征': ['exp_x_fans', 'champ_x_fans', 'wins_x_age', 'season_x_fans'],
    '行业特征': ['ind_Entertainment', 'ind_Sports', 'ind_Media'],
    '核心特征': ['seasons_std', 'avg_place_std', 'age_std', 'followers_std', 
               'season_std', 'week_std', 'ind_Entertainment', 'ind_Sports']
}

feature_subsets = {f'全部特征({n_features})': list(range(n_features))}
for group_name, group_features in metrics_groups.items():
    # 查找当前有效特征的索引
    indices = [feature_names.index(f) for f in group_features if f in feature_names]
    if indices:
        feature_subsets[f'{group_name}({len(indices)})'] = indices

subset_results = {}
for name, indices in feature_subsets.items():
    # Double check indices are valid
    indices = [i for i in indices if i < n_features]
...
ax1 = axes[0, 0]
folds = range(1, 11)
ax1.bar(np.array(folds) - 0.15, cv_r2_judge, width=0.3, label='CV Score (Judge)', color='steelblue')
ax1.bar(np.array(folds) + 0.15, cv_r2_place, width=0.3, label='Final Rank', color='coral')
...
ax1.set_title('10-Fold Cross-Validation R² Distribution')
ax1.legend()
ax1.set_xticks(folds)

# 3.2 残差分布
ax2 = axes[0, 1]
ax2.hist(residuals_judge, bins=50, alpha=0.7, label='Judge Scores', color='steelblue', density=True)
ax2.hist(residuals_place, bins=50, alpha=0.7, label='Final Rank', color='coral', density=True)
# 添加正态曲线
x_range = np.linspace(-10, 10, 100)
ax2.plot(x_range, stats.norm.pdf(x_range, 0, residuals_judge.std()), 'b--', label='Normal Distribution')
...
ax3.get_lines()[0].set_color('steelblue')
ax3.get_lines()[1].set_color('red')
ax3.set_xlabel('Theoretical Quantiles')
ax3.set_ylabel('Ordered Values')
ax3.set_title('Residual Q-Q Plot (Judge Scores)')

# 3.4 噪声鲁棒性
ax4 = axes[1, 0]
ax4.plot([n*100 for n in noise_levels], robustness_results['judge'], 'o-', label='Judge Scores', color='steelblue')
ax4.plot([n*100 for n in noise_levels], robustness_results['place'], 's-', label='Final Rank', color='coral')
ax4.axhline(y=base_r2_judge, color='steelblue', linestyle='--', alpha=0.5, label=f'Baseline(Judge)={base_r2_judge:.3f}')
ax4.axhline(y=base_r2_place, color='coral', linestyle='--', alpha=0.5, label=f'Baseline(Rank)={base_r2_place:.3f}')
ax4.set_xlabel('Noise Level (%)')
ax4.set_ylabel('R² Score')
ax4.set_title('Noise Robustness Analysis')
ax4.legend()
ax4.grid(True, alpha=0.3)

# 3.5 训练比例稳定性
ax5 = axes[1, 1]
ax5.plot([r*100 for r in train_ratios], ratio_results['judge'], 'o-', label='Judge Scores', color='steelblue')
ax5.plot([r*100 for r in train_ratios], ratio_results['place'], 's-', label='Final Rank', color='coral')
ax5.set_xlabel('Training Set Ratio (%)')
ax5.set_ylabel('Test Set R²')
ax5.set_title('Data Split Stability')
ax5.legend()
ax5.grid(True, alpha=0.3)

# 3.6 Bootstrap R²分布
ax6 = axes[1, 2]
ax6.hist(bootstrap_r2_judge, bins=30, alpha=0.7, label='Judge Scores', color='steelblue')
ax6.hist(bootstrap_r2_place, bins=30, alpha=0.7, label='Final Rank', color='coral')
# Compute global max for vlines to set a dummy label if needed, or just let legend handle colors
ax6.axvline(x=np.percentile(bootstrap_r2_judge, 2.5), color='steelblue', linestyle='--', label='95% CI')
...
ax6.set_title('Bootstrap R² Distribution (95% CI)')
...
ax.set_title('Residuals vs Fitted (Judge Scores)')
ax.legend()
...
ax.set_title('Standardized Residuals (Judge Scores)')
ax.legend()
...
ax.axhline(y=0.05, color='red', linestyle='--', alpha=0.5, label='Significance Threshold (0.05)')
...
""")
# 动态获取键名
key_all = next((k for k in subset_results.keys() if k.startswith('全部特征')), None)
key_core = next((k for k in subset_results.keys() if k.startswith('核心特征')), None)

if key_all and key_core and subset_results[key_all]['judge_r2'] != 0:
    ratio = subset_results[key_core]['judge_r2'] / subset_results[key_all]['judge_r2'] * 100
    print(f"   - {key_core}可达到{key_all}约{ratio:.1f}%的解释力")
else:
    print(f"   - 无法计算核心特征与全部特征的比例 (Key mismatch or zero R2)")

print("""   - 结论: 关键特征贡献稳定，模型结构合理
""")
