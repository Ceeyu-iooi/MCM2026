...
fig, axes = plt.subplots(1, 2, figsize=(14, 8), subplot_kw=dict(projection='polar'))
colors = ['#3498db', '#e74c3c', '#2ecc71', '#9b59b6']
method_names = list(methods.keys())

legend_map = {
    '固定权重(50:50)': 'Fixed Weight',
    '百分比法': 'Percentage',
    '排名法': 'Rank Method',
    'KMDR': 'KMDR'
}
...
ax1 = axes[0]
metrics_labels = ['Satisfaction\nBalance', 'Noise\nRobustness', 'Condorcet\nEfficiency']
...
for i, name in enumerate(method_names):
    ...
    vals += [vals[0]]
    label_en = legend_map.get(name, name)
    ax1.plot(angles, vals, label=label_en, color=colors[i], linewidth=2)
    ax1.fill(angles, vals, color=colors[i], alpha=0.1)

ax1.set_xticks(angles[:-1])
ax1.set_xticklabels(metrics_labels, fontsize=10)
ax1.set_ylim(0, 1.1)
ax1.set_title('Fairness Metrics Comparison', y=1.08, fontsize=14, fontweight='bold')
ax1.legend(loc='upper center', bbox_to_anchor=(0.5, -0.1), ncol=2)

ax2 = axes[1]
method_labels = [legend_map.get(n, n).replace(' Method', '').replace(' Weight', '') for n in method_names]
num_vars2 = len(method_labels)
angles2 = np.linspace(0, 2 * np.pi, num_vars2, endpoint=False).tolist()
angles2 += [angles2[0]]

j_sats = [satisfaction[m]['评委满意度'] for m in method_names]
j_sats += [j_sats[0]]
ax2.plot(angles2, j_sats, label='Judge Satisfaction', color='#3498db', linewidth=2, linestyle='-')
ax2.fill(angles2, j_sats, color='#3498db', alpha=0.1)

f_sats = [satisfaction[m]['观众满意度'] for m in method_names]
f_sats += [f_sats[0]]
ax2.plot(angles2, f_sats, label='Fan Satisfaction', color='#e74c3c', linewidth=2, linestyle='--')
ax2.fill(angles2, f_sats, color='#e74c3c', alpha=0.1)

ax2.set_xticks(angles2[:-1])
ax2.set_xticklabels(method_labels, fontsize=10)
ax2.set_ylim(0.8, 1.0)
ax2.set_title('Judge vs Fan Satisfaction Profile', y=1.08, fontsize=14, fontweight='bold')
