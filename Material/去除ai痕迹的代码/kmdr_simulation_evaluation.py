...
colors_line = ['#2ecc71', '#f39c12', '#e74c3c']

ax.plot(groups, stabilities, marker='o', linestyle='-', linewidth=2.5, markersize=10, 
        color='#3498db', label='Volatility Trend')

for i, (group, val, color) in enumerate(zip(groups, stabilities, colors_line)):
    ax.plot(group, val, marker='o', markersize=12, color=color, 
            markeredgecolor='white', markeredgewidth=2)
    ax.text(x=group, y=val + 0.05, s=f'{val:.2f}', 
            ha='center', va='bottom', fontsize=11, fontweight='bold', color=color)

ax.set_ylabel('Ranking Volatility (Std Dev)', fontsize=12)
ax.set_title('Ranking Stability by Player Strength (Line Chart)', fontsize=14, fontweight='bold')
