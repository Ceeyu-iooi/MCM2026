...
for ax, (outcome, results, title) in zip(axes, [
    ('week_rank', results_judge, 'Judge Score Model'),
    ('placement', results_place, 'Final Placement Model')
]):
    ...
    bars1 = ax.bar(x - width/2, r2_train, width, label='Training R²', color='steelblue', alpha=0.8)
    bars2 = ax.bar(x + width/2, r2_cv, width, label='Cross-Validation R²', color='coral', alpha=0.8,
    ...
    y_max = max(r2_train + r2_cv) + (max(r2_cv_std) if r2_cv_std else 0) + 0.05
    ax.set_ylim(0, min(1.0, y_max))
    ax.axhline(y=0.35, color='green', linestyle='--', alpha=0.5, label='Baseline Model')
    ax.grid(axis='y', alpha=0.3)
    if outcome == 'week_rank':
...
ax.set_xlabel('OLS Coefficients')
ax.set_ylabel('Bayesian Ridge Coefficients')
ax.set_title('Judge Score: OLS vs Bayesian Ridge')
ax.grid(alpha=0.3)
...
ax.set_xlabel('OLS Coefficients')
ax.set_ylabel('Bayesian Ridge Coefficients')
ax.set_title('Final Placement: OLS vs Bayesian Ridge')
ax.grid(alpha=0.3)
...
ax.set_title('Judge Score: Random Forest Feature Importance (Top 10)')
...
ax.set_yticklabels([f.replace('_std', '') for f in top_features], fontsize=9)
ax.set_xlabel('Feature Importance')
ax.set_title('Final Placement: Random Forest Feature Importance (Top 10)')
ax.grid(axis='x', alpha=0.3)
...
for row, (outcome, results, y_true, title) in enumerate([
    ('week_rank', results_judge, y_judge, 'Judge Score'),
    ('placement', results_place, y_place, 'Final Placement')
]):
...
# Figure 9: Model CV RMSE Comparison
...
bars1 = ax.bar(x - width/2, rmse_judge, width, label='Judge Score', color='steelblue')
bars2 = ax.bar(x + width/2, rmse_place, width, label='Final Placement', color='coral')

ax.set_ylabel('Cross-Validation RMSE')
ax.set_title('Model Prediction Error Comparison')
ax.set_xticks(x)
