...
def evaluate_kmdr(df, alpha=0.4, beta=0.6, w_early=0.75, w_mid=0.60, w_late=0.50):
    ...
    for season in finals['season'].unique():
        ...
        if len(s_data) > 0:
            ...
            if real_champ is None and 'Placement' in s_data.columns:
                real_champ = s_data.loc[s_data['Placement'].idxmin(), 'celebrity_name']
...
b = ((~kmdr_correct) & fixed_correct).sum()
c = (kmdr_correct & (~fixed_correct)).sum()
