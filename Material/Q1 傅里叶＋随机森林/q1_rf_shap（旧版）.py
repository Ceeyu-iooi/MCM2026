...
# 加载清洗后的数据
df = pd.read_csv('c:/Users/yhl20/Desktop/Problem C/processed_dwts_cleaned_ms08.csv')
...
# 使用TreeExplainer（对树模型最高效）
explainer = shap.TreeExplainer(rf_clf)
shap_values = explainer.shap_values(X_eval)

# 对于二分类，shap_values是一个列表[class0, class1]或ndarray
...
# 加载TAN预测结果进行相关性分析
try:
    tan_pred_df = pd.read_csv('c:/Users/yhl20/Desktop/Problem C/q1/q1_tan_predicted_fan_votes.csv')
...
output_dir = 'c:/Users/yhl20/Desktop/Problem C/q1/'
...
plt.tight_layout()
plt.savefig(output_dir + 'fig1_rf_feature_importance.png', dpi=150, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(output_dir + 'fig2_rf_shap_summary.png', dpi=150, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(output_dir + 'fig3_rf_confusion_matrix.png', dpi=150, bbox_inches='tight')
plt.close()
...
ax.set_ylim([0, 1])
plt.tight_layout()
plt.savefig(output_dir + 'fig4_rf_roc_curve.png', dpi=150, bbox_inches='tight')
plt.close()
print("  [4/6] fig4_rf_roc_curve.png")

# 图5: 观众投票分数分布
fig, ax = plt.subplots(figsize=(10, 6))
ax.hist(fan_vote_score[~eliminated_mask], bins=30, alpha=0.7, label='Stay (Survived)', color='green', density=True)
ax.hist(fan_vote_score[eliminated_mask], bins=30, alpha=0.7, label='Eliminated', color='red', density=True)
...
ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(output_dir + 'fig5_rf_fan_vote_distribution.png', dpi=150, bbox_inches='tight')
plt.close()
...
if tan_available:
    ...
    colors = ['green' if e == 0 else 'red' for e in y]
    ax.scatter(tan_fan_votes, fan_vote_score, c=colors, alpha=0.5, s=20)
    ...
    ax.legend(['Perfect Agreement', 'Stay', 'Eliminated'], loc='upper left')
    ax.grid(alpha=0.3)
    ax.set_xlim([0, 100])
    ax.set_ylim([0, 100])
    
    # 添加相关性文本
    ax.text(5, 90, f'Spearman rho = {corr:.4f}\np-value < 0.001', fontsize=11, 
            bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
    
    plt.tight_layout()
    plt.savefig(output_dir + 'fig6_rf_tan_comparison.png', dpi=150, bbox_inches='tight')
    plt.close()
...
# 保存预测结果CSV
result_df.to_csv(output_dir + 'q1_rf_predicted_fan_votes.csv', index=False)
...
with open(output_dir + 'q1_rf_metrics.json', 'w', encoding='utf-8') as f:
...
with open(output_dir + 'q1_rf_report.txt', 'w', encoding='utf-8') as f:
    f.write(report)
