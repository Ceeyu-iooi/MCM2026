...
ax1 = axes[0]
cv_data_rf = [cv_results['RF'][m]['scores'] for m in metrics]
bp1 = ax1.boxplot(cv_data_rf, labels=['Accuracy', 'Precision', 'Recall', 'F1', 'AUC'], patch_artist=True)
for patch in bp1['boxes']:
    patch.set_facecolor('steelblue')
    patch.set_alpha(0.7)
ax1.set_ylabel('Score')
ax1.set_title('Random Forest: 10-Fold Cross-Validation')
ax1.grid(axis='y', alpha=0.3)
ax1.set_ylim([0, 1])

ax2 = axes[1]
cv_data_gnb = [cv_results['GNB'][m]['scores'] for m in metrics]
bp2 = ax2.boxplot(cv_data_gnb, labels=['Accuracy', 'Precision', 'Recall', 'F1', 'AUC'], patch_artist=True)
for patch in bp2['boxes']:
    patch.set_facecolor('coral')
    patch.set_alpha(0.7)
ax2.set_ylabel('Score')
ax2.set_title('Gaussian Naive Bayes: 10-Fold Cross-Validation')
