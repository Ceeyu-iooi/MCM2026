# -*- coding: utf-8 -*-
"""
Q4: KMDR (Kemeny-Metric Distortion Rank) Model
基于学术论文理念的融合模型
...
参考文献：
- Boehmer et al. (2022) "Rank Aggregation Using Scoring Rules"
- Charikar et al. (2023) "Breaking the Metric Voting Distortion Barrier"
- Yang et al. (2024) "Designing Digital Voting Systems for Citizens"
- Ebadian et al. (2024) "Optimized Distortion and Proportional Fairness in Voting"
"""
...
class KMDRModel:
    """
    KMDR模型 - 融合Kemeny聚合与度量扭曲理论
    
    核心机制：
    1. Borda位置评分 - 将分数转换为位置得分
    2. Kemeny距离 - 计算排名间的肯德尔距离
    3. Condorcet矩阵 - 两两比较胜负关系
    4. 度量扭曲惩罚 - 对评委-观众分歧施加惩罚
    5. 动态权重 - 基于赛程阶段调整
    """
    
    def __init__(self, 
                 alpha=0.4,           # 度量扭曲惩罚强度
                 beta=0.6,            # Condorcet权重
                 gamma=0.7,           # 评委初始权重
                 adaptive=True):      # 是否动态调整
        self.name = "KMDR"
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
    ...
    def _condorcet_matrix(self, judge_rank, fan_rank):
        """
        Condorcet矩阵：两两比较获胜次数
        C[i][j] = 1 if i beats j in both rankings
        """
        n = len(judge_rank)
        C = np.zeros((n, n))
    ...
    def _metric_distortion(self, judge_scores, fan_scores, judge_rank, fan_rank):
        """
        度量扭曲惩罚：评委-观众排名不一致性的度量
        基于Charikar et al. (2023)的度量扭曲理论
        """
        n = len(judge_scores)
        ...
        # 计算度量空间中的距离
        # d(i,j) = |j_norm[i] - f_norm[i]| 表示选手i在两个度量空间中的不一致性
    ...
    def score(self, j, f, week, contestants):
        ...
        condorcet_score = condorcet.sum(axis=1)  # 每个选手的总胜场数
        
        # 6. 度量扭曲惩罚
        distortion = self._metric_distortion(j, f, judge_rank, fan_rank)
        
        # 7. 综合得分
        # Borda基础分 + Condorcet共识奖励 - 度量扭曲惩罚
        final_scores = (
            borda_fused + 
...
class KMDRAggressive:
    ...
    def __init__(self):
        self.name = "KMDR (激进版)"
        self.alpha = 0.8      # 更强扭曲惩罚
        self.beta = 0.8       # 更重视Condorcet
        self.gamma = 0.7
        self.adaptive = True
...
print("""
╔═══════════════════════════════════════════════════════════════════╗
║           KMDR (Kemeny-Metric Distortion Rank) Model             ║
║                基于学术论文理念的融合模型                          ║
╠═══════════════════════════════════════════════════════════════════╣
║                                                                   ║
║ 【理论基础】                                                       ║
║                                                                   ║
║  1. Borda计数 (Borda Count)                                       ║
║     • 将分数转换为位置评分                                         ║
║     • 最高分n-1分，次高n-2分，依此类推                             ║
║     • 参考: 经典社会选择理论                                       ║
║                                                                   ║
║  2. Kemeny聚合 (Kemeny Rank Aggregation)                          ║
║     • 寻找与所有输入排名Kendall距离之和最小的共识排名              ║
║     • K = argmin Σ d_τ(R, R_i)                                   ║
║     • 参考: Boehmer et al. (2022) "Rank Aggregation Using        ║
║              Scoring Rules", arXiv:2209.08856                     ║
║                                                                   ║
║  3. Condorcet方法 (Condorcet Method)                              ║
║     • 两两比较确定选手间的相对优势                                 ║
║     • C[i][j] = 1 if i beats j in both rankings                  ║
║     • 共识强度：两者同意则奖励，分歧则惩罚                         ║
║                                                                   ║
║  4. 度量扭曲 (Metric Distortion)                                  ║
║     • 在度量空间中最小化社会成本                                   ║
║     • D(i) = 0.6×|j_norm(i) - f_norm(i)| + 0.4×|rank_diff(i)|   ║
║     • 参考: Charikar et al. (2023) "Breaking the Metric Voting   ║
║              Distortion Barrier", arXiv:2306.17838               ║
║                                                                   ║
║  5. 比例公平性 (Proportional Fairness)                            ║
║     • 动态调整评委-观众权重                                        ║
║     • Week 1-3: w_j=0.75 (评委导向)                              ║
║     • Week 4-6: w_j=0.60 (中等平衡)                              ║
║     • Week 7+:  w_j=0.50 (完全平衡)                              ║
║     • 参考: Ebadian et al. (2024) "Optimized Distortion and      ║
║              Proportional Fairness in Voting", ACM ToE           ║
║                                                                   ║
║ 【综合评分公式】                                                   ║
║                                                                   ║
║   Score = Borda_fused + β×Condorcet_score - α×n×Distortion      ║
║                                                                   ║
║   其中:                                                           ║
║   • Borda_fused = w_j×Borda_judge + w_f×Borda_fan               ║
║   • Condorcet_score = Σ_j C[i][j] (总胜场数)                     ║
║   • Distortion = 度量空间不一致性惩罚                             ║
║   • α = 0.4 (标准版), 0.8 (激进版)                               ║
║   • β = 0.6 (标准版), 0.8 (激进版)                               ║
║                                                                   ║
║ 【关键创新】                                                       ║
