# -*- coding: utf-8 -*-
"""
Q4: KMDR Model Sensitivity Analysis
Analyze how key parameters (α, β) affect model performance
"""
...
def kmdr_score(group, alpha=0.4, beta=0.6):
    """Calculate KMDR score with adjustable α and β"""
    n = len(group)
    if n < 2:
...
def simulate_season_with_params(season_data, alpha=0.4, beta=0.6):
    """Simulate a single season with given parameters"""
...
def evaluate_params(df, alpha, beta):
    """Evaluate performance metrics for given parameters"""
    results = {
        'alpha': alpha,
        'beta': beta,
        'champion_strength': [],
    ...
    for season in seasons:
        ...
        # Elimination rationality (first 3 eliminations)
        ...
        for celeb in controversial:
            if celeb in ranking:
                ...
                if celeb_rank > n_players * 0.5:  # Placed in bottom half
...
for idx, (metric, title) in enumerate(metrics_to_plot):
    ...
    ax.scatter([6.5], [4.5], marker='*', s=300, c='blue', edgecolors='white', linewidth=2, zorder=5)
...
# Alpha sensitivity (fixing β=0.6)
...
# Mark stability region (composite score >= 95% of max)
